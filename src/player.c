...
struct raycaster_player *player_create(struct raycaster_window *window, struct raycaster_map *map, double position_x, double position_y, double rotation) {
	...
	*player = (struct raycaster_player) { window, map, position_x, position_y, rotation };
	window_get_mouse_position(window, &player->old_mouse_x, &player->old_mouse_y);
	return player;
}
...
void player_update(struct raycaster_player *player) {
	...
	window_get_mouse_position(player->window, &mouse_x, &mouse_y);
	...
	if (window_is_key_down(player->window, INPUT_KEY_RIGHT)) player->rotation += player_turn_speed;
	if (window_is_key_down(player->window, INPUT_KEY_LEFT)) player->rotation -= player_turn_speed;

	// Player movement input
	if (window_is_key_down(player->window, INPUT_KEY_W)) { player->velocity_x += cos(player->rotation) * player_accel; player->velocity_y += sin(player->rotation) * player_accel; }
	if (window_is_key_down(player->window, INPUT_KEY_S)) { player->velocity_x -= cos(player->rotation) * player_accel; player->velocity_y -= sin(player->rotation) * player_accel; }
	if (window_is_key_down(player->window, INPUT_KEY_D)) { player->velocity_x -= sin(player->rotation) * player_accel; player->velocity_y += cos(player->rotation) * player_accel; }
	if (window_is_key_down(player->window, INPUT_KEY_A)) { player->velocity_x += sin(player->rotation) * player_accel; player->velocity_y -= cos(player->rotation) * player_accel; }

	// Check for collision before applying velocity
	if (map_get_wall(player->map, (int)(player->position_x + player->velocity_x), (int)(player->position_y + player->velocity_y)) == 0) {
